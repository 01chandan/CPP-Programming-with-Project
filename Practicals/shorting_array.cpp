...
int main(void)
{
    ...
    //bubble short
    cout<<"\n\n1. Shorted Accending"<<endl<<"2. Shorted Decending"<<endl<<"3. Exit";
    cout<<"\n\nEnter your Choice: ";cin>>choice;
    if(choice == 1)
    {
    for(i=0; i<(user-1); i++) //user -1 because we compare 4 element of array only
    {
        for(j=0; j<(user-1-i); j++)
        {
    ...
    cout<<"\n\nOutpur Ready...\n\n";
    cout<<"Shorted Accending: ";
    for(i=0; i<user; i++)
    {   Sleep(500);
        cout<<"|"<<setw(2)<<arruser[i]<<"| "; //setw(3) set the size of output
    }
    }else if(choice == 2)
    {
    for(i=0; i<(user-1); i++) //user -1 because we compare 4 element of array only
